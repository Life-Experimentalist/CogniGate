...
func TestGW9_AC5_MetaIsCheapEnoughToPoll(t *testing.T) {
	...
	//
	// Its own tenant, narrowed to a single request per second. /v1/meta is
	// outside the rate limit — a document a client is told to poll cannot be
	// metered, or it stops answering exactly when the client most needs to know
	// what the deployment is doing — and a bucket of one is what turns that from
	// an assumption into an assertion. The calls are unpaced for the same reason:
	// a pause between them would let a metered route pass too.
	tn := newTenant(t, "gw9-ac5")
	...
	for i := 0; i < calls; i++ {
		started := time.Now()
		resp, err := c.try(http.MethodGet, "/v1/meta", tn.Key, nil)
		elapsed := time.Since(started)
		if err != nil {
			t.Fatalf("call %d: %v", i+1, err)
		}
		if resp.Status != http.StatusOK {
			t.Fatalf("call %d answered %d, want 200; on a bucket of one that is /v1/meta being metered, which GW-9 forbids", i+1, resp.Status)
		}
