...
- `openapi.yaml` now covers the full GW-1..GW-14 surface (33 paths across
  both planes) and is generated by `scripts/gen_openapi.py`, so it is
  edited there and never by hand. What is still outstanding is the other
  half: GW-10's suite asserts response shapes in Go rather than against
  the document, so the two can drift without anything failing.
